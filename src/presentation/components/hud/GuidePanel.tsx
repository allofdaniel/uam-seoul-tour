...
export default function GuidePanel() {
  ...
  // 음성 대화 모드: 좌측 상단 컴팩트 패널
  if (isVoiceMode && (isNarrating || narrationText)) {
    return (
      <div className="absolute top-6 left-6 z-20 max-w-sm animate-fadeIn">
        <div className="bg-indigo-950/70 backdrop-blur-md border border-indigo-500/30 rounded-2xl p-4 shadow-2xl">
          {/* 헤더 */}
          ...
          <div className="text-gray-200 text-sm leading-relaxed min-h-[3rem]">
            {isNarrating && !narrationText ? (
              ...
              <>
                {displayText}
                {isTyping && <span className="inline-block w-1 h-4 bg-indigo-400 animate-pulse ml-0.5" />}
              </>
            )}
          </div>
          ...
          {highlightKeyword && (
            <div className="mt-2">
              <span className="inline-block px-2 py-1 bg-indigo-500/20 border border-indigo-500/30 rounded-full text-indigo-400 text-xs">
                #{highlightKeyword}
              </span>
            </div>
          )}
  ...
  // 일반 자동 안내 모드: 우측 사이드 패널 (main 디자인)
