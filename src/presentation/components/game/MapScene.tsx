  ...
  useEffect(() => {
    ...
    const initViewer = () => {
      ...
      poiData.forEach((poi) => {
        const e = viewer.entities.add({
          ...
          position: Cesium.Cartesian3.fromDegrees(
            poi.lon,
            poi.lat,
            Math.max(poi.altitude_m || 30, 30) + 60
          ),
          point: {
            pixelSize: 12,
      ...
      viewer.scene.preRender.addEventListener(() => {
        ...
        // UAM 방향 (heading → CesiumJS 방향 + 모델 보정)
        const hpr = new Cesium.HeadingPitchRoll(
          Cesium.Math.toRadians(state.heading),
          Cesium.Math.toRadians(state.pitch * 0.3),
        ...
        const cameraRange = Math.max(100, 120 + state.speed_kmh * 0.5);
        const cameraHeading = Cesium.Math.toRadians(smoothHeading) + Math.PI;
        const cameraPitch = Cesium.Math.toRadians(-18);
        ...
        poiEntities.forEach((entity, id) => {
          if (entity.point) {
            if (visitedIds.includes(id)) {
              entity.point.color = Cesium.Color.LIME.withAlpha(0.9);
              entity.point.outlineColor = Cesium.Color.GREEN;
            }
          }
        });
      });
